...
> **Note:** Changing to `q35` will rename network interfaces inside the VM (e.g. `ens18` to `enp6s18`). Fix this after the VM boots — see Phase 2.3.

### 2.2 Add GPU to the Docker VM

**Via Proxmox web UI** (must be logged in as root or Administrator):

1. Go to **Datacenter** -> select your Docker VM -> **Hardware**
...
### On the Docker VM — Remove NVIDIA Drivers and Toolkit
